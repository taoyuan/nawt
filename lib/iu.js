...
async function resolve(iface) {
  ...
  const networks = (await iu.lshw({class: 'network'})).filter(n => n.capabilities.wireless === 'Wireless-LAN');
...
/**
 *
 * @param {String} [iface] empty for all interfaces
 * @return {Promise<void>}
 */
async function ifstatus(iface) {
  if (iface) {
    iface = await resolve(iface);
  }
  if (iface) {
    return PromiseA.fromCallback(cb => ifconfig.status(iface, cb));
  } else {
    return PromiseA.fromCallback(cb => ifconfig.status(cb));
  }
}

async function up(iface) {
  iface = await resolve(iface);
  await exec(`sudo ip link set ${iface} up`);
}

async function down(iface) {
  iface = await resolve(iface);
  await exec(`sudo ip link set ${iface} down`);
}

async function reset(iface, delay = 1000) {
  await down(iface);
